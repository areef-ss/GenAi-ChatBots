...
if user_prompt:
    ...
    retrieval_chain=create_retrieval_chain(retriver,documnet_chain)

    start=time.process_time()
    responce=retrieval_chain.invoke({'input':user_prompt})
    print(f"Response time:{time.process_time()-start}")
    end=time.process_time()
    st.write(responce['answer'])

    with st.expander("Documnet similiarity search"):
